...
class Entity():
    def __init__(self,player, x, y, type, speedX = 0):
        pygame.sprite.Sprite.__init__(self)
        self.type = type
        self.speedX = speedX
        if self.speedX <= 0:
            self.image = sprites[self.type]
        else:
            self.image = pygame.transform.flip(sprites[self.type], True, False)
    ...
    def update(self):
        ...
        if self.type != 'minecart' and self.type != 'boat': # Não perder tempo
            return
        self.rect.centerx += self.speedX
        #condição para o carrinho que sair de um lado da screen, voltar para a outra extremidade
