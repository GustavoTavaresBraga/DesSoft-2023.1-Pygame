...
class TelaJogo:
    ...
    def nova_fileira(self, y=0):
        ...
        for i in range(10):
            Entity(self.player, i * 50 + 25, y, block) #criando as entidades
        for i in range(10):
            if block == 'water' and random.random() < 0.25:
                Entity(self.player, i * 50 + 25, y, 'boat', speed*direction)
                gerouObstaculo = True
            if block == 'rails' and random.random() < 0.25:
                Entity(self.player, i * 50 + 25, y, 'minecart', speed*direction)
                gerouObstaculo = True
